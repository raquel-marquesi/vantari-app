...
| **10** — Validador Email + Lista Inteligente | ✅ | **Estava marcado ⏳ mas já estava feito** — achado 22/09/2026 checando o banco antes de reimplementar, ver aviso no topo deste arquivo. Lista Inteligente = segmentos dinâmicos de `/segments` (já existiam). Validador: `core.classify_email` (migration 31/07) classifica sintaxe + domínio descartável + email genérico via trigger automático (`core.persons.email_status`), com selo em `/leads` e filtro/preset em `/segments`; `send-campaign` pula quem está `invalid`. 22/09/2026: somada checagem de domínio via DNS (MX/A) — Edge Function `validate-email-domain`, cron a cada 10min (`validate-email-domain-every-10-min`, mesmo padrão pg_cron+pg_net do `sync-meta-leads`) — pega domínio digitado errado/inexistente que a heurística sozinha não pegava |
