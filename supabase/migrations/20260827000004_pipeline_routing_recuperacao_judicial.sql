-- ═══════════════════════════════════════════════════════════════════
-- Prioridade 4 do plano de unificação de leads — roteia negócios da
-- campanha "Recuperação Judicial — Varejo" pro pipeline dedicado
-- (criado em 20260825000001, hoje com 0 negócios porque nada rotava
-- ainda). Só daqui pra frente — sem backfill de negócios antigos.
--
-- ATENÇÃO — drift encontrado (2026-08-27): a versão de
-- public.trg_form_submission_to_lead REALMENTE ATIVA no banco já
-- estava com o roteamento por slug aplicado (comparado via
-- pg_get_functiondef), mas isso NÃO existia em nenhuma migration
-- deste repositório — foi aplicado direto em produção, fora do
-- controle de versão, em algum momento entre 20260817000005 (última
-- versão commitada, sem roteamento) e agora. Esta migration só
-- RECAPTURA essa versão já viva (sem mudança funcional nela) pra
-- fechar o drift entre repo e banco. As duas peças que realmente
-- faltavam — o gatilho de auto-detecção por regex e o /ingest da
-- Nina — são a mudança de verdade aqui.
-- ═══════════════════════════════════════════════════════════════════

-- ---------- 1) Sincroniza o repo com a versão já viva em produção ----------
-- (nenhuma mudança funcional — só captura o que já está rodando)
...
-- ---------- 2) Gatilho de auto-detecção por regex (Nina) — este SIM
--    ainda não tinha roteamento nenhum. attributes.campanha já chega
--    e já é gravado em core.person_attributes via /ingest (confirmado:
--    core.set_person_attributes aceita qualquer chave) — só faltava
--    ler esse valor aqui e mapear pro pipeline. ----------
